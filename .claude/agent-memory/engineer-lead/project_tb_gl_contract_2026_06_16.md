...
### LNS-415 update (2026-06-26): old per-account `/ledger` endpoint REMOVED → `getGeneralLedger` is the canonical per-account ledger-entries source.
Re-verified live spec 2026-06-26: `GET /accounting/accounts/{account_id}/ledger` **no longer exists** (only `/balance` = getCoaAccountBalance remains under that prefix, query start_date/end_date both OPTIONAL). The accounting `ledger-account` source's `listEntries` was repointed to `getGeneralLedger` (`GET /accounting/reports/general-ledger/{account_id}`, query `from`/`to` BOTH REQUIRED). Consequences for any code feeding `LedgerEntryModel` from this endpoint: (1) entries are at `result.data.lines` (NOT `result.data`, which is the GeneralLedgerData OBJECT); (2) pagination is TOP-LEVEL `result.meta`, while `result.data.meta` is the report meta (tenant_id/account_id/from/to/...) — same-name-two-surface cross-wire trap; (3) `GeneralLedgerLine` has NO per-line `account_id` (it's only in `data.meta.account_id`) — inject it before `fromJson` or the entity's `string accountId` silently becomes undefined. getGeneralLedger returns 200/400/403 — NO 404; empty period = 200 with `lines: []`.
