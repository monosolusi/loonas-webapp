...
**Why:** On LNS-374 the spec drew Laba Rugi as a 2-section (Pendapatan/Beban) table, fully spec'd a non-cash disclosure note with concrete fields {id, description, amount}, and assumed a second compare fetch. The real contract was a fixed multi-bucket single-step ledger, an empty `{}` non-cash item schema (v1 always `[]`), and compare nested in one POST — so EL had to reconcile/walk back several sections. **Recurred on LNS-376:** a full prose-only CALK viewer spec was committed while the per-note item shape was still an open EL question; EL's raw-spec parse then found Note 3 is a `line_items` table (not prose), forcing a complete spec revision. Because it recurred, this rule was promoted into the always-on operating principles in `ui-designer.md` — consult this memory before committing any content-shape-dependent layout.
