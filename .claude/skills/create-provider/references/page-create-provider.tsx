// Canonical example: Pattern C — page-level create/form provider with:
// - form field state
// - debounced preview fetch
// - submit action that triggers mutation, revalidates SWR key, shows toast
// Source: src/app/(authenticated)/productions/create/_providers/production-create-provider.tsx
...
export function ProductionCreateProvider({ children }: ProductionCreateProviderProps) {
  ...
  const handleSubmit = async (): Promise<ProductionRecordEntity | null> => {
    ...
    try {
      ...
      // Revalidation is the CALLER's job — mutation hook doesn't know which lists to refresh.
