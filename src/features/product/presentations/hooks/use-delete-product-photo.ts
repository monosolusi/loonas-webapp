...
type DeletePhotoTriggerParams = {
  productId: string;
  photoId: string;
};

type DeletePhotoFetcherParams = DeletePhotoTriggerParams & {
  clerk: ReturnType<typeof useClerk>;
};

async function DeletePhotoFetcher(_: string, { arg }: { arg: DeletePhotoFetcherParams }): Promise<void> {
  ...
  const result = await productRepository.deletePhoto(arg.productId, arg.photoId, session.data);
  if (result instanceof DataFailed) throw result.error;
}

export function useDeleteProductPhoto() {
  return useSWRMutationClerk("delete-product-photo", DeletePhotoFetcher);
}
