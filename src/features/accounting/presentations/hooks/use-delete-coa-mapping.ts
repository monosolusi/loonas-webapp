...
import { DeleteCoaMappingUseCase, DeleteCoaMappingUseCaseParams } from "@/features/accounting/domain/usecases/delete-coa-mapping.usecases";
...
async function DeleteCoaMappingFetcher(_: string, { arg }: { arg: DeleteCoaMappingFetcherParams }): Promise<void> {
  ...
  const uc = new DeleteCoaMappingUseCase(repo, sessionRepo);
  const result = await uc.execute(new DeleteCoaMappingUseCaseParams(arg.id));
