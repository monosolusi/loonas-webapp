...
export class ClosePeriodUseCase implements UseCase<DataState<ClosePeriodResult>, ClosePeriodUseCaseParams> {
  ...
  public async execute(params: ClosePeriodUseCaseParams): Promise<DataState<ClosePeriodResult>> {
    try {
      ...
      return new DataSuccess(await this.closePeriod(params, session));
    } catch (err) {
  ...
  private async closePeriod(params: ClosePeriodUseCaseParams, session: SessionEntity): Promise<ClosePeriodResult> {
    const result = await this.repo.close({ id: params.id, idempotencyKey: params.idempotencyKey, reason: params.reason }, session);
