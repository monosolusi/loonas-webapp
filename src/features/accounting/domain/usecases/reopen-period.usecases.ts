...
export class ReopenPeriodUseCase implements UseCase<DataState<AccountingPeriodEntity>, ReopenPeriodUseCaseParams> {
  ...
  public async execute(params: ReopenPeriodUseCaseParams): Promise<DataState<AccountingPeriodEntity>> {
    try {
      ...
      return new DataSuccess(await this.reopenPeriod(params, session));
    } catch (err) {
  ...
  private async reopenPeriod(params: ReopenPeriodUseCaseParams, session: SessionEntity): Promise<AccountingPeriodEntity> {
    const result = await this.repo.reopen({ id: params.id, reason: params.reason, idempotencyKey: params.idempotencyKey }, session);
