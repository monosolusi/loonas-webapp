...
type CreateCoaMappingLineInput = {
  accountId: string;
...
type CreateCoaMappingInput = {
  entityType: string;
  entityId?: string | null;
  lines: CreateCoaMappingLineInput[];
};

export class CreateCoaMappingUseCaseParams {
  constructor(public readonly params: CreateCoaMappingInput) {}
}

export class CreateCoaMappingUseCase
  implements UseCase<DataState<CoaMappingEntity>, CreateCoaMappingUseCaseParams>
{
  constructor(
  ...
  public async execute(params: CreateCoaMappingUseCaseParams): Promise<DataState<CoaMappingEntity>> {
    try {
      ...
      return await this.repo.create(params.params, session);
    } catch (err) {
