 ...
 *
 * The server owns the real gate and rejects an incompatible pair with 422
 * `CASH_CATEGORY_DIRECTION_MISMATCH` (category create/update) or 422
 * `CASH_CATEGORY_ACCOUNT_TYPE_MISMATCH` (cash-entry-settings update). Never use this mapping
 * to decide whether a request may be sent — use it only to narrow an account list before it
 * reaches a picker, so a mismatch stays a server-authoritative error and not a silent FE
 * divergence.
 */
