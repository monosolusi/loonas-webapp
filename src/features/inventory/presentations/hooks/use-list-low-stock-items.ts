...
import { ListLowStockItemsUseCase, ListLowStockItemsUseCaseParams } from "@/features/inventory/domain/usecases/list-low-stock-items.usecases";
...
async function ListLowStockItemFetcher([_, params]: [string, ListStockItemFetcherParams]) {
  const sessionRepository = new SessionRepositoryImpl(new ClerkSessionService({ clerk: params.clerk }));
  const stockItemRepository = new StockItemRepositoryImpl(new StockItemServiceImpl(new HttpRequest()));
  const useCase = new ListLowStockItemsUseCase(stockItemRepository, sessionRepository);
  const result = await useCase.execute(
    new ListLowStockItemsUseCaseParams({
      page: params.page,
      limit: params.limit,
    }),
  );
...
export function useListLowStockItems(params: UseListStockItemsParams = {}): UseListStockItemsReturnType {
  const clerk = useClerk();
  const { data, isLoading, error } = useSWR(
    [INVENTORY_SWR_KEYS.LIST_LOW_STOCK_ITEMS, { ...params, clerk }],
    ListLowStockItemFetcher,
  );
