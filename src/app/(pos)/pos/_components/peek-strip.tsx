...
import { PeekStripBayarButton } from "@/app/(pos)/pos/_components/peek-strip-bayar-button";
import { PeekStripInWizardLabel } from "@/app/(pos)/pos/_components/peek-strip-in-wizard-label";

export function PeekStrip() {
  const { items, total } = usePosCart();
  const { checkoutStep, drawerOpen, toggleDrawer } = usePosUI();
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-30 flex h-16 items-center gap-x-3 border-t border-neutral-200 bg-white px-4 lg:hidden">
      ...
      {inWizard ? <PeekStripInWizardLabel /> : <PeekStripBayarButton />}
    </div>
  );
}
