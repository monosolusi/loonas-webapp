  ...
  return (
    <div className="flex flex-row gap-x-4">
      ...
      {summary.unpaidCount > 0 ? (
        <Link href="/invoices/incoming/unpaid" className={`${warningBase} transition-shadow hover:shadow-sm`}>
          {unpaidCard}
        </Link>
      ) : (
      ...
      {/* Telah Dibayar */}
      {summary.paidCount > 0 ? (
        <Link href="/invoices/incoming/paid" className={`${successBase} transition-shadow hover:shadow-sm`}>
          {paidCard}
        </Link>
      ) : (
