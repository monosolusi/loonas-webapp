...
export function BankCombobox({ selectedBank, setSelectedBank }: BankComboboxProps) {
  ...
  return (
    ...
    >
      ...
      <div className="relative mt-2">
        <ComboboxInput
          className="focus:outline-primary-default block w-full rounded-md bg-white py-1.5 pr-12 pl-3 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 sm:text-sm/6"
          ...
          <ComboboxOptions className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-hidden sm:text-sm">
            {filteredBank.map((bank) => (
              <ComboboxOption
                key={bank.id}
                value={bank}
                className="group data-focus:bg-primary-default relative cursor-default py-2 pr-9 pl-3 text-gray-900 select-none data-focus:text-white data-focus:outline-hidden"
              >
                <span className="block truncate group-data-selected:font-semibold">{bank.name}</span>

                <span className="text-primary-default absolute inset-y-0 right-0 hidden items-center pr-4 group-data-focus:text-white group-data-selected:flex">
