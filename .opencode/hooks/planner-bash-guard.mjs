#!/usr/bin/env node
// PreToolUse guard for the `planner` primary agent: Bash edition.
//
// Enforces the planner's read-only mandate for shell commands. Every Bash
...
const OLLAMA_MODEL = process.env.PLANNER_BASH_GUARD_MODEL || "gemma4:31b-cloud";
...
try {
  ...
  deny("planner bash-guard: could not parse the PreToolUse payload — blocking the Bash call.");
}

const command = input?.tool_input?.command;
if (!command || typeof command !== "string") {
  deny("planner bash-guard: tool call had no command — blocking the Bash call.");
}

const systemPrompt = `You are a strict shell-command security classifier for a planner agent that must NEVER modify files on disk.
...
try {
  ...
  if (!res.ok) {
    deny(
      `planner bash-guard: Ollama returned HTTP ${res.status} — blocking the Bash call to stay safe.`,
    );
  }
  ...
  deny(
    `planner bash-guard: could not reach Ollama at ${OLLAMA_HOST} (${err.name}: ${err.message}) — blocking the Bash call.`,
  );
}
...
try {
  ...
  deny(
    `planner bash-guard: Ollama response was not valid JSON (${responseText.slice(0, 200)}) — blocking the Bash call.`,
  );
}

if (classification?.is_write_or_edit === true) {
  deny(
    `planner is READ-ONLY and must never write, edit, or delete files. ` +
      `Delegate implementation work to the appropriate agent after planning. Blocked command: ${command.slice(0, 200)}`,
  );
}
