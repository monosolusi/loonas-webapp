...
- [ ] AC0 — Orchestration vs. single-agent decision. Assess whether the work should be executed by a single agent or requires a multi-agent orchestrator. Justify the decision using **complexity** (number of steps, cross-feature/cross-layer touchpoints, parallelizable work, need for specialized lanes such as PM/EL/UI/SWE/QA) and **impact** (production risk, user-facing scope, architectural surface area, number of project conventions affected, risk of regression). State the conclusion explicitly in the plan; do not delegate — this is an assessment/recommendation, not an action.
- [ ] AC1 — ...
- [ ] AC2 — ...
- [ ] AC3 — ...
